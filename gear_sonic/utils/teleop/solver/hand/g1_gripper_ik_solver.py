"""IK solver that maps fingertip pinch gestures to Inspire RH56 joint targets.

Uses Manus/Pico pinch detection (which fingertip is touching the thumb) and
outputs joint angles for the 6 Inspire drives in the actuator order defined by
``inspire_hand_spec.JOINT_ORDER`` (pinky, ring, middle, index, thumb_bend,
thumb_rot). Angles are in radians where 0 is fully open and positive is flexion;
values stay within the URDF joint limits. The grasp deepens as the pinch moves
from index toward pinky (index -> ... -> full grasp). These amplitudes are
intentionally simple and meant to be tuned on hardware.
"""
...
def _pinch_target(finger_data, dist_threshold: float = 0.05):
    """Detect which fingertip (if any) is pinching the thumb from glove data.

    Returns one of "index", "middle", "ring", "pinky", or None.
    """
